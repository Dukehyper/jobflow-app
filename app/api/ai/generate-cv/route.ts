...
import { generateText, MAX_TOKENS_CV } from '@/lib/ai/client'
import { buildCVPrompt } from '@/lib/anthropic/prompts'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = buildCVPrompt(masterProfile, job.description ?? '', validatedCvType)

    const generatedCV = await generateText(prompt, MAX_TOKENS_CV)
    ...
    if (updateError) {
      return NextResponse.json(
        { data: null, error: 'Failed to save generated CV' },
        { status: 500 }
      )
    }

    // 8. Return result
    return NextResponse.json({ data: generatedCV, error: null })
  } catch (err) {
    console.error('[generate-cv] Unhandled error:', err)
    return NextResponse.json(
